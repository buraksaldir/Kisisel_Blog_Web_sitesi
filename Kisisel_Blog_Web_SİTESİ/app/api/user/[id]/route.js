...
export async function PATCH(req, res) {
  ...
  try {
    ...
    if (user?._id.toString() !== decodedToken._id.toString()) {
      return NextResponse.json(
        { msg: "Verilerini yalnızca yazar güncelleyebilir" },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({ message: "PATCH Hata" }, { status: 500 });
  }
}
...
export async function DELETE(req, res) {
  ...
  try {
    ...
    if (user?._id.toString() !== decodedToken._id.toString()) {
      return NextResponse.json(
        { msg: "Yalnızca yazar kendi verileri silebilir" },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({ message: "Hatayı sil" }, { status: 500 });
  }
}
